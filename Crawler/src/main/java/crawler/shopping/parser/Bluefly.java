...
public class Bluefly implements ParserImpl {
	...
	public void run(XmlData xmlData, String startDate) {
		...
		HasAttributeFilter attributeFilter = new HasAttributeFilter("class", "retail-price");
		...
		if(nodeList.size() > 0){
			...
			for(int i = 0; i < nodeList.size(); i++){
				if(nodeList.elementAt(i).getText().contentEquals("em") == true){
					this.price = nodeList.elementAt(i + 1).getText();
		...
		attributeFilter = new HasAttributeFilter("itemprop", "price");
