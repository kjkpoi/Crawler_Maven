...
public class Gap implements ParserImpl {
	...
	private String getDiscountPrice(String html, String code){
		String str = code +"'].price";
	...
	private String getOriginalPrice(String html, String code){
		String str = code +"'].retailPrice";
		int start = html.indexOf(str) + str.length();
		start = html.indexOf('\'', start) + 1;
		int end = html.indexOf('\'', start);
		return html.substring(start , end);
	...
	private ArrayList<String> getSizeList(String html, String code){
		ArrayList<String> list = new ArrayList<>();
		String str = "sizeHash['" + code + "'] = new Array(";
		int start = html.indexOf(str) + str.length();
		start = html.indexOf('\'', start) + 1;
		int end = html.indexOf(");", start);
		...
		while(token.hasMoreTokens()){
			str = token.nextToken().replace("'", "").trim();
			list.add(str);
		}
		return list;
	}
	...
	public String getProductName(String html){
		...
		HasAttributeFilter attributeFilter = new HasAttributeFilter("name", "keywords");
	...
	public boolean isAvailable(String html){
		if(html.contains("We're sorry, but the item or outfit you selected is out of stock. Click here to continue shopping from your previous page.") == true){
			return false;
		}
		return true;
	}
